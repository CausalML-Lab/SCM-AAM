...
%         case 'c95', error = (data_std./sqrt(size(data,1))).*1.50;
%     end
    error = (data_std./sqrt(size(data,1)))*2;
    % Plotting the result
    %figure(options.handle);
    x_vector = [options.x_axis', fliplr(options.x_axis')];
   % error = error./1.8;
   % error(1:500) = error(400:899) 
    patch = fill(x_vector, [data_mean+error,fliplr(data_mean-error)], options.color_area);
    set(patch, 'edgecolor', 'none');
    set(patch, 'FaceAlpha', options.alpha);
    hold on;

 plot(options.x_axis, data_mean,options.marker, 'color', options.color_line, ...
        'LineWidth', options.line_width,"MarkerSize",8.4,'MarkerIndices',[20:ceil(options.x_axis(end)/5):options.x_axis(end)]);
